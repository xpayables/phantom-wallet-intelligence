...
acts = st.sidebar.multiselect("Recommended action", sorted(df.recommended_action.unique()), placeholder="All actions",
    help="Narrow the target list to one campaign's audience (e.g., just Win-back, or Card/CASH) to size or export it.")
segs = st.sidebar.multiselect("Segment", sorted(df.segment_name.unique()), placeholder="All segments",
    help="Slice by behavioral persona to explore a group, or cross with an action. Descriptive only: it doesn't change the priority score or actions.")
